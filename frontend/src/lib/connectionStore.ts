...
function sortByUpdatedAtDesc<T extends { updatedAt: number }>(
  records: T[],
): T[] {
...
export const sessionCreationDefaults = writable<SessionCreationDefaults | null>(
  null,
);

export async function fetchSessionCreationDefaults(): Promise<void> {
  const { sessionCreationDefaults: defaults } =
    await getSessionCreationDefaults();
...
export async function updateSessionCreationDefaults(input: {
  ...
  const { sessionCreationDefaults: updated } =
    await putSessionCreationDefaults(input);
