...
async function request<T>(
  ...
  // Never let a malformed record escape as an uncaught ZodError (which blanks
  // the whole view) — surface it through the app's normal error channel.
  const parsed = options.schema.safeParse(payload)
  if (!parsed.success) {
    const first = parsed.error.issues[0]
    const where = first ? first.path.join('.') || '(root)' : '(unknown)'
    console.error(`mcpscope: response validation failed for ${path}`, parsed.error.issues)
    throw new AppError(
      `Backend response for ${path} failed validation at "${where}": ${first?.message ?? 'unknown issue'}`,
      'internal',
      response.status,
...
export function fetchLmConnectionModelDetails(
  ...
  return request('/api/lm-connections/models/details', {
    method: 'POST',
    body: { baseUrl, modelKey, providerType, apiKey },
