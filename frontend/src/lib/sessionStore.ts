...
// Session summaries are snake_case (backend-canonical), so this store keeps
// its own variant of format.ts's sortByUpdatedAtDesc.
    ...
    if (isJobActive) {
      ...
      if (cachedEvents.length > 0) {
        ...
        for (let i = cachedEvents.length - 1; i >= 0; i--) {
          if (TERMINAL_TYPES.has(cachedEvents[i]!.event.type)) {
            startIdx = i + 1
            break
          }
        }
        ...
        if (eventsToReplay.length > 0) {
          ...
          for (const { event, jobKind } of eventsToReplay) {
            // Route prelude (init-job) events to the prelude handler — mirror
            // the live router in executionStore, keyed on the emitting job's
            // kind (part-committed exists in both vocabularies). Feeding init
            // events to the turn handler used to fall through to a phantom
            // "turn-failed" and kill the freshly-primed stream.
            if (isPreludeExecutionEvent(event, jobKind)) {
