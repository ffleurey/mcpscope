    ...
    <div class="user-bubble">
      ...
      {#if tokenCount !== null}
        <div class="message-meta">
          <span class="token-pill pill-end">{fmtTokens(tokenCount)}</span>
        </div>
      {/if}
    </div>
      ...
      {#each tools as tool}
        ...
        <details class="tool-item">
          <summary class="tool-summary">
            ...
            {#if estTokens !== null}
              <span class="token-pill pill-end">~{estTokens.toLocaleString()} tokens</span>
            {/if}
          </summary>
...
<style>
  ...
  /* Layout-only companion class — never shadow the global .token-pill. */
  .pill-end {
    margin-left: auto;
  }
