<script lang="ts">
    ...
    const userPart = $derived(
        sortedParts.find((p) => p.partType === "user-message") ?? null,
    );
    const ungroupedParts = $derived(
        sortedParts.filter((p) => p.roundId === null),
    );
    const assistantContentParts = $derived(
        sortedParts.filter((p) => p.partType === "assistant-content"),
    );
    const toolCallCount = $derived(
        sortedParts.filter((p) => p.partType === "tool-call").length,
    );
    const hasReasoning = $derived(
        sortedParts.some((p) => p.partType === "assistant-reasoning"),
    );
...
<section class="compact-turn">
    ...
    {#if mode === "chat" && turnIsComplete}
            ...
            <div class="chat-toggle-row">
                ...
                >
                    ...
                    <span class="chat-toggle-stats">
                        {sortedRounds.length} round{sortedRounds.length !== 1
                            ? "s"
                            : ""}
                        {#if toolCallCount > 0}· {toolCallCount} tool call{toolCallCount !==
                            1
                                ? "s"
                                : ""}{/if}
        ...
        {#if lastRound}
            {@const lastSnapshot =
                contextSnapshotsByRound?.get(lastRound.id) ?? null}
    ...
    {:else}
        ...
        {#each sortedRounds as round (round.id)}
            ...
            {@const roundStream = roundStreamsByRound.get(round.id) ?? null}
            {@const roundSnapshot =
                contextSnapshotsByRound?.get(round.id) ?? null}
            <section class="compact-round">
                ...
                {#if mode === "inspect"}
                    ...
                    <div class="compact-round-meta">
                        <span class="compact-round-label"
                            >Round {round.roundIndex + 1}</span
                        >
                        <span class="compact-round-status"
                            >{round.finishReason ?? round.status}</span
                        >
                ...
                <div class="compact-round-parts">
                    ...
                    {:else if round.status === "streaming"}
                        <div class="round-streaming-hint">
