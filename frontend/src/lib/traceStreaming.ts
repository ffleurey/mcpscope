...
// SHORTCUT: deriveTranscriptEntries / deriveContextEntries / the round
// snapshot below re-implement backend selectors of the same names so
// streaming upserts can render without a round-trip — including compaction
// visibility rules (strippedByCompactionAtTurnId ordering + a heuristic
// step-id fallback), which are runtime semantics that can silently drift from
// the backend. Paying this back = have the stream carry the derived views (or
// re-fetch them per round) and delete the duplication; until then, any change
// to the backend selectors must be mirrored here.
