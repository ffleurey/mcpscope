...
It is intentionally small. It is the **canonical mcpscope model** used by persistence, API, UI inspect workflows, and the CLI.
...
- benchmark **LLM evaluation**: an optional per-case rubric judged by a separate model after a run. An evaluation reuses the analysis model directly — it spawns one `session_analysis` session (workflow kind `benchmark_evaluation`) per completed run-session whose case has a rubric (rubric-less sessions are skipped), parented to that run-session — so it adds no new session type or parent rule (see [BENCHMARK.md](../BENCHMARK.md))
...
**Numbering is positional, per subtype**: a turn segment's number is the turn's 1-based
position among the session's turns (`1T`, `2T`, …), while workflow (`W`) and compaction (`C`)
segments are numbered by their 1-based position in the session's full ordered step list (turns
included). The type tag is therefore part of the identity — `2C` and `2T` can coexist — and IDs
are unique within each subtype. Session IDs stay short, stable, and human-readable.
