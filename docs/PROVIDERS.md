...
Provider-specific logic is concentrated in `backend/src/services/provider/`, with thin per-provider clients under `backend/src/services/` and some route-level dispatch (model listing, load/unload gates) that delegates to them. Adding a fourth provider means extending this module — see [Adding a new provider](#adding-a-new-provider).
...
- **Tradeoff:** two sessions requesting *different* models on one instance will thrash (each turn re-asserts its model) — inefficient, and not fully race-free: the mutex covers the swap sequence but is released before the chat request itself, so a concurrent swap can still evict the model in that window (surfacing as a failed request, recoverable by re-running). Accepted; LM Studio is single-model under a VRAM limit anyway, and one session at a time is the intended workflow.
...
| **OpenRouter** | Non-streaming responses don't include `usage` | Estimates from text + tool definitions length using `estimateTokensFromText`. Also degrades to this estimate when the probe itself is rejected with HTTP 400 (OpenRouter rejects the `max_tokens: 1` probe when the prompt would trigger a tool call). Non-400 errors (auth/transport/5xx) still propagate. |
...
3. OpenAI-compatible `/v1/models` endpoint (OpenRouter)
