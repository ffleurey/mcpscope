...
The scheduler is the only execution owner. No route or operation directly runs a model turn, analysis step, or initialization outside the scheduler in normal runtime flow. **Known exception (an architecture-level shortcut):** `launchAnalysis` awaits the analysis session's initialization inline at launch time, so judge/analysis-session init is not an abortable scheduler job — a stop request cannot interrupt it (run sessions' init, by contrast, is a scheduler job). Paying this back means enqueueing analysis init like session init.
...
- `backend/src/routes/` groups HTTP routes by concern:
	...
	- `benchmarkRoutes.ts` for benchmark suite/case/run/evaluation routes and run control
...
This composition extends cleanly to **benchmark LLM evaluation**, which deliberately reuses the *analysis* subsystem rather than standing up a parallel judging path. An evaluation coordinator launches one `benchmark_evaluation` **analysis session** per run-session (via the same `executeAnalysisLaunch` + scheduler-await path used for any analysis), and each judge session runs the standard analysis loop against the read-only `/mcp/analysis` endpoint — the judge is given the target session ID and inspects it on demand via `mcpscope_inspect` (starting from the inspect summary; no trace is pre-injected into its context), with a structured-output verdict written as an artifact. Benchmark evaluation and session analysis are therefore **one mechanism, not two**: the rubric judge is just another registered `WorkflowStep`/workflow kind, so improvements to the analysis framework (prompting, evidence loading, scheduling, persistence) accrue to both and the two cannot drift into separate implementations.
...
- `POST /api/sessions`
- `POST /api/sessions/from-defaults` — create a session from backend-owned defaults (accepts `wait` to block until initialization is terminal)
- `POST /api/sessions/:sessionId/initialize` — SSE initialization flow
- `POST /api/sessions/:sessionId/analyze` — launch a session-analysis / benchmark-evaluation child session
...
- `POST /api/sessions/:sessionId/turns/start` — detached turn start for polling workflows (accepts `wait` to block until the turn is terminal)
