...
- **npm** — publishes the package (CLI + bundled backend/frontend) to the npm registry
...
work). Requires an **`NPM_TOKEN`** repository secret (an npm automation token with publish rights
for the `mcpscope` package). `prepublishOnly` runs `build:all`, so the published tarball ships
the built `cli/dist` + `backend/dist` + `frontend/dist` (the `files` allow-list in package.json).
