...
- `npm run check:electron` — Electron TypeScript check

Lint and format:

- `npm run lint` — frontend ESLint (`npm run lint:backend` / `npm run lint:cli` / `npm run lint:electron` for the others)
...
backend turn pipeline without live infrastructure.
...
- **MCP HTTP endpoint smoke test** (`backend/src/mcp/mcp.test.ts`): sends a
  ...
  contains every `mcpscope_*` tool name matching the backend catalog.
...
The `session_analysis` workflow has one focused backend regression that should stay
