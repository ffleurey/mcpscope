...
- benchmark tables: `benchmarks`, `benchmark_cases`, `benchmark_runs`, `benchmark_evaluations` (documented under [Benchmark Tables](#benchmark-tables) below; omitted from the ER diagram for readability)
...
Benchmarks add four tables (see [BENCHMARK.md](../BENCHMARK.md) for the feature reference). IDs are type-tagged and hierarchical: `B-7K3M` (benchmark), `B-7K3M.3` (case 3), `R-9QX4` (run), `E-2F8P` (evaluation). A run produces one normal primary session per case × repetition, parented to the run id via the `sessions` parent columns above; an evaluation produces one `benchmark_evaluation` analysis session per completed run-session whose case has a rubric (rubric-less run-sessions are skipped), parented to that run-session.
...
- `benchmark_runs` — an immutable execution snapshot, **independent** of its source benchmark (no FK / no cascade): `id`, `benchmark_id` (soft reference), `benchmark_name` (snapshot for display), `status` (`pending`/`running`/`paused`/`stopped`/`complete`/`error` — `paused`/`stopped` are resumable rest states set by user pause/stop), `model_config_id`, `mcp_profile_ids_json` (effective selection, resolved at launch), `cases_json` (snapshot of the selected cases: `{sourceCaseId, name, prompt, expectedToolsCalled, expectedToolsNotCalled, rubric}` — the rubric is snapshotted too, so an evaluation judges against the run-time rubric), `repetitions`, `max_tool_rounds` (`INTEGER NOT NULL DEFAULT 20`; the per-session tool-round budget — see `DEFAULT_MAX_TOOL_ROUNDS`), `sessions_json` (the produced `{sessionId, sourceCaseId, repetition, status, error}` mapping; per-session `status` is `running`/`complete`/`error`/`cancelled`, the last being a session interrupted by a stop; `error` holds the failure reason for an errored session, e.g. unreachable MCP or unavailable model), `error`, `created_at`, `updated_at`, `started_at`, `completed_at`.
- `benchmark_evaluations` — one LLM judging pass over a run (FK → `benchmark_runs`, `ON DELETE CASCADE`): `id`, `run_id`, `judge_model_config_id`, `judge_temperature` (`REAL`, **nullable**: the judge sampling temperature chosen for the pass; `NULL` ⇒ send no temperature, i.e. provider default. The Zod/API layer default when a temperature is supplied is `0.2` — small but non-zero so a retry of a looping judge can escape — but this is not a column default), `status` (`pending`/`running`/`paused`/`stopped`/`complete`/`error`), `sessions_json` (one entry per **judged** run-session — rubric-less run-sessions are omitted: `{runSessionId, analysisSessionId, status}`, status `running`/`complete`/`error`/`cancelled` — links each scored session to the `benchmark_evaluation` analysis session that judged it), `error`, `created_at`, `updated_at`. A thin grouping record holding the judge config (model + temperature); **scores are not stored** — they are computed on read from the judge sessions' verdict artifacts (in `artifacts`).

Indexes: `idx_benchmark_cases_benchmark`, `idx_benchmark_runs_benchmark`, `idx_benchmark_evaluations_run`. The report and evaluation scores are computed on read from the produced sessions (never the live cases). Lifecycles: deleting a **benchmark** cascades to its **cases** (FK) and, in application code, to its **runs** — each run taking its produced sessions, judge sessions, and `benchmark_evaluations` with it (a run is only reachable through its benchmark, so leaving it behind would orphan it and leak its sessions into `list`; the cascade is atomic and refuses if a run is active). Deleting a **run** removes its produced sessions explicitly (generic untyped parent → no FK cascade) and cascades to its `benchmark_evaluations`. **Editing** a benchmark or case never alters a past run, its report, or its evaluations — the run is a frozen snapshot; only deleting the benchmark removes the run.

## Constraints and Indexes

- Enumerated lifecycle columns are enforced with `CHECK (...)` constraints in `backend/src/persistence/schema.ts`.
- `steps` declares `UNIQUE (session_id, parent_step_id, child_index)`, but SQLite treats NULL as
  distinct in unique indexes and top-level steps have a NULL `parent_step_id` — so for them the
  constraint is inert and `child_index` uniqueness is maintained by the writers
  (`getNextChildIndex`), not the schema.
...
The schema version is `6`, stored in `schema_meta.schema_version` (defined as `SCHEMA_VERSION` in `backend/src/domain/model.ts`). History: `1` → `2` added `benchmark_cases.rubric_json` + the `benchmark_evaluations` table (LLM evaluation); `2` → `3` added `benchmark_evaluations.judge_temperature`; `3` → `4` added the run-control lifecycle statuses (benchmark run/eval `paused`/`stopped`, per-session `cancelled`) plus the run-session `error` field; `4` → `5` added `benchmark_runs.max_tool_rounds` (per-session tool-round budget); `5` → `6` made `benchmark_evaluations.judge_temperature` nullable (`NULL` ⇒ send no temperature, i.e. provider default). There is no migration path (pre-1.0): opening a database stamped with a different schema version **fails loudly at startup**, naming the file, both versions, and the way out (move/delete the file, or run the mcpscope version that created it). The version is also surfaced on the `/api/domain-model` endpoint.
