...
export async function runCreate(opts: CreateOptions): Promise<void> {
  ...
  renderCreateText(result, opts.wait);
}

function renderCreateText(result: CreateResult, waited = false): void {
  ...
  if (waited && session.init_status === "ready") {
    process.stdout.write(
      `\nRun 'mcpscope send ${session.id} <prompt>' to start a turn.\n`,
    );
  } else if (waited) {
    process.stdout.write(
      `\nInitialization ended with '${session.init_status}'. Run 'mcpscope status ${session.id}' for details.\n`,
    );
  } else {
    process.stdout.write(
