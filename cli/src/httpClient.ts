...
/**
 * fetch wraps the real network error ("fetch failed" → cause: AggregateError
 * of per-address connect errors) — dig out the first concrete message so
 * ECONNREFUSED is visible (and triggers the `mcpscope serve` hint).
 */
...
/**
 * POST over plain node:http(s) with NO client-side timeout. `--wait` on
 * create/send holds a single request open until the init/turn is terminal;
 * the global `fetch` (undici) aborts such requests after its default 300 s
 * headersTimeout — shorter than a long local-model turn — surfacing as a
 * misleading "Cannot reach backend: fetch failed" while the turn keeps
...
export async function cliBenchmarkDelete(
  baseUrl: string,
...
export async function cliBenchmarkDeleteRun(
  baseUrl: string,
  body: { run_id: string },
...
export async function cliBenchmarkDeleteEvaluation(
  baseUrl: string,
