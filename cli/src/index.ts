...
import {
  ...
  parseBenchmarkDeleteArgs,
  runBenchmarkDelete,
  parseBenchmarkDeleteRunArgs,
  runBenchmarkDeleteRun,
  parseBenchmarkDeleteEvaluationArgs,
...
function reportFatal(err: unknown): never {
  ...
  if (err instanceof OperationError) {
    if (wantsJson) {
      ...
      // --json consumers get the documented `code` (CLI.md error tables).
...
// the CLI. argv[1] is realpath-resolved because npm installs the bin as a
