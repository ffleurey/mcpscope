...
export function initializeSchema(connection: BackendConnection): void {
  connection.exec(`
    ...
    CREATE TABLE IF NOT EXISTS steps (
      ...
      -- NOTE: inert for top-level steps — SQLite treats NULL as distinct in
      -- unique indexes and every current insert uses parent_step_id NULL, so
      -- uniqueness of (session_id, child_index) is maintained by the writers
      -- (getNextChildIndex), not by this constraint. Enforcing it for NULL
      -- parents needs an expression index, i.e. a schema-version bump.
...
/**
 * Fail loudly when the database file was written by a different schema
 * version. There is no migration path: tables are created with IF NOT EXISTS
 * and validateSchema checks column presence only, so silently reusing an old
 * file would pass validation and then fail later on constraint-level changes
 * (e.g. the v5→v6 judge_temperature NOT NULL relaxation) as raw
