...
describe('configStore: CRUD', () => {
  it('creates, reads, updates, and deletes LM connections', () => {
    ...
    expect(store.listLmConnections().find((c) => c.id === 'lm-new')?.name).toBe('New Connection')
  ...
  it('creates, reads, updates, and deletes model configs', () => {
    ...
    expect(store.listModelConfigs().find((c) => c.id === 'mc-new')?.temperature).toBe(0.8)
  ...
  it('creates, reads, updates, and deletes MCP server profiles', () => {
    ...
    expect(store.listMcpServerProfiles().find((p) => p.id === 'mp-new')?.defaultEnabled).toBe(true)
...
describe('configStore: file flush', () => {
  it('writes config back to the file after upsert', () => {
    ...
    expect(raw.lm_connections.find((c: { id: string }) => c.id === 'lm-after')?.name).toBe(
      'After Upsert',
    )
  })
...
describe('configStore: end-to-end config lifecycle', () => {
  it('writes config via API, then reloads from file on next startup', async () => {
    ...
    expect(store2.listLmConnections().find((c) => c.id === 'lm-local')?.name).toBe(
      'Local LM Studio',
    )
    expect(store2.listLmConnections().find((c) => c.id === 'lm-second')?.name).toBe(
      'Second Connection',
    )
    ...
    expect(store2.getSessionCreationDefaults().defaultModelConfigId).toBe('mc-deepseek')
  })
})
