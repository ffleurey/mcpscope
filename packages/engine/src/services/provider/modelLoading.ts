...
function withInstanceLock<T>(baseUrl: string, fn: () => Promise<T>): Promise<T> {
 ...
 * leave loading alone.
 */
export async function ensureModelReady(params: EnsureModelReadyParams): Promise<void> {
  const { baseUrl, apiKey, providerType, modelKey, contextSize, autoSwap } = params
  ...
  await withInstanceLock(baseUrl, async () => {
    ...
    if (loadedKeys.length === 1 && loadedKeys[0] === modelKey && !targetNeedsReload) {
      return
    }
