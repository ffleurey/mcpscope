...
import type { SchedulerEvent, SchedulerEventListener } from './runtime/schedulerTypes.js'
...
import { initializeMcpSession, listMcpTools, callMcpTool } from './services/mcp/httpClient.js'
...
import { deleteSessionOperation, type DeleteInput, type DeleteResult } from './operations/delete.js'
import { renameSessionOperation, type RenameInput, type RenameResult } from './operations/rename.js'
import { abortSessionOperation, type AbortInput, type AbortResult } from './operations/abort.js'
...
import type { LmStudioConnection, ModelConfig, McpServerProfile } from './domain/configuration.js'
...
export interface Engine {
  ...
  /** Abort a session's active turn or dequeue its pending job. */
...
export async function createEngine(options: CreateEngineOptions): Promise<Engine> {
  ...
  return {
    ...
    deleteSession: (input) => deleteSessionOperation.execute(opCtx, input),
    renameSession: (input) => renameSessionOperation.execute(opCtx, input),
    abortSession: (input) => abortSessionOperation.execute(opCtx, input),
    getTrace: (sessionId) => {
