...
export function applyContextCompaction(
  ...
  // One transaction for the whole sequence (step INSERT, part strips, step
  // state, turn columns) — a crash mid-way must not leave parts marked
  // 'stripped' while the turn's compaction columns were never written.
  // runInTransaction is savepoint-aware, so the caller's transaction (if any)
