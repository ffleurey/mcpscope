...
- `benchmark_runs` — an immutable execution snapshot, **independent** of its source benchmark (no FK / no cascade): `id`, `benchmark_id` (soft reference), `benchmark_name` (snapshot for display), `status` (`pending`/`running`/`paused`/`stopped`/`complete`/`error` — `paused`/`stopped` are resumable rest states set by user pause/stop), `model_config_id`, `mcp_profile_ids_json` (effective selection, resolved at launch), `cases_json` (snapshot of the selected cases: `{sourceCaseId, name, prompt, expectedToolsCalled, expectedToolsNotCalled, rubric}` — the rubric is snapshotted too, so an evaluation judges against the run-time rubric), `repetitions`, `sessions_json` (the produced `{sessionId, sourceCaseId, repetition, status}` mapping; per-session `status` is `running`/`complete`/`error`/`cancelled`, the last being a session interrupted by a stop), `error`, `created_at`, `updated_at`, `started_at`, `completed_at`.
