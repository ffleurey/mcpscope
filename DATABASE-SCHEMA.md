...
- `benchmark_runs` — an immutable execution snapshot, **independent** of its source benchmark (no FK / no cascade): `id`, `benchmark_id` (soft reference), `benchmark_name` (snapshot for display), `status` (`pending`/`running`/`paused`/`stopped`/`complete`/`error` — `paused`/`stopped` are resumable rest states set by user pause/stop), `model_config_id`, `mcp_profile_ids_json` (effective selection, resolved at launch), `cases_json` (snapshot of the selected cases: `{sourceCaseId, name, prompt, expectedToolsCalled, expectedToolsNotCalled, rubric}` — the rubric is snapshotted too, so an evaluation judges against the run-time rubric), `repetitions`, `sessions_json` (the produced `{sessionId, sourceCaseId, repetition, status, error}` mapping; per-session `status` is `running`/`complete`/`error`/`cancelled`, the last being a session interrupted by a stop; `error` holds the failure reason for an errored session, e.g. unreachable MCP or unavailable model), `error`, `created_at`, `updated_at`, `started_at`, `completed_at`.
- `benchmark_evaluations` — one LLM judging pass over a run (FK → `benchmark_runs`, `ON DELETE CASCADE`): `id`, `run_id`, `judge_model_config_id`, `judge_temperature` (the judge sampling temperature chosen for the pass; default `0.2` — small but non-zero so a retry of a looping judge can escape), `status` (`pending`/`running`/`paused`/`stopped`/`complete`/`error`), `sessions_json` (per run-session: `{runSessionId, analysisSessionId, status}`, status `running`/`complete`/`error`/`cancelled` — links each scored session to the `benchmark_evaluation` analysis session that judged it), `error`, `created_at`, `updated_at`. A thin grouping record holding the judge config (model + temperature); **scores are not stored** — they are computed on read from the judge sessions' verdict artifacts (in `artifacts`).
