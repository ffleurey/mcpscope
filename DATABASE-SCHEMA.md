...
- `benchmark_evaluations` — one LLM judging pass over a run (FK → `benchmark_runs`, `ON DELETE CASCADE`): `id`, `run_id`, `judge_model_config_id`, `status` (`pending`/`running`/`complete`/`error`), `sessions_json` (per run-session: `{runSessionId, analysisSessionId, status}` — links each scored session to the `benchmark_evaluation` analysis session that judged it), `error`, `created_at`, `updated_at`. A thin grouping record; **scores are not stored** — they are computed on read from the judge sessions' verdict artifacts (in `artifacts`).
...
The schema version is `2`, stored in `schema_meta.schema_version` (defined as `SCHEMA_VERSION` in `backend/src/domain/model.ts`). It was bumped from `1` when benchmark evaluation added `benchmark_cases.rubric_json` and the `benchmark_evaluations` table. With no migration path the version is informational (surfaced on the `/api/system` endpoint); an out-of-date database is started empty rather than migrated.
