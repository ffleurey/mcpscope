...
export function deleteBenchmarkRunEntry(db: BackendDatabase, runId: string): void {
  ...
  if (run.status === "pending" || run.status === "running" || run.status === "paused") {
    throw new OperationError(
      `Benchmark run "${runId}" is ${run.status}; stop it before deleting.`,
      "benchmark_run_active",
    );
  }
...
export function deleteBenchmarkEvaluationEntry(
  ...
  if (
    ...
    throw new OperationError(
      `Benchmark evaluation "${evaluationId}" is ${evaluation.status}; stop it before deleting.`,
      "benchmark_evaluation_active",
    );
  }
  for (const judge of evaluation.sessions) {
    try {
...
async function runBenchmarkCoordinator(
  ...
  try {
    ...
    const reconciled: BenchmarkRunRecord = {
      ...
      // A resumed run is no longer completed: clear any stale terminal stamp
      // (a restart-reconciled 'stopped' run may carry one).
      completedAt: null,
      error: null,
      updatedAt: now(),
    };
    ...
    // Resolve the run's model/MCP configuration ONCE per coordinator pass and
    // reuse it for every repetition — resolving per repetition let a mid-run
    // config edit change later repetitions despite the run's snapshot story
    // (a deleted config still fails loudly, here, before any session starts).
    ...
    if (supersede.length > 0) {
      ...
      // Evaluations referencing superseded sessions would otherwise point at
      // deleted records (empty case ids, passes reading as incomplete) — drop
      // those entries from every evaluation of this run.
      for (const evaluation of listBenchmarkEvaluationsByRun(
        db.connection,
        runId,
      )) {
        const kept = evaluation.sessions.filter(
          (s) => !supersedeSet.has(s.runSessionId),
        );
        if (kept.length !== evaluation.sessions.length) {
...
async function runOneRepetition(
  ...
  snapshot: BenchmarkRunCaseSnapshot,
  rep: number,
  resolved: ResolvedPrimarySessionInputs,
  controller: RunController | undefined,
): Promise<string> {
  ...
  try {
    ...
    // SHORTCUT: rethrowing makes one hard per-session failure (createSession /
    // enqueue) abort the whole run, while the evaluation coordinator catches
    // per-session errors and continues — the two mirrored coordinators
    // disagree. Paying this back = pick one semantics (continue, recording the
    // session as errored, is the soft-failure rule the header comment states)
 ...
 */
/**
 * Run-sessions eligible for judging: completed, and their case snapshot carries
 * a rubric. Sessions of rubric-less cases are deliberately skipped — you rubric
 * the cases where answer quality matters, and the pass still completes.
 */
export function rubricJudgeableSessions(run: BenchmarkRunRecord): {
  judgeable: BenchmarkRunRecord["sessions"];
  skippedNoRubric: number;
} {
  const hasRubric = new Map(
    run.cases.map((c) => [c.sourceCaseId, c.rubric.length > 0]),
  );
  const completed = run.sessions.filter((s) => s.status === "complete");
  const judgeable = completed.filter(
    (s) => hasRubric.get(s.sourceCaseId) === true,
  );
  return { judgeable, skippedNoRubric: completed.length - judgeable.length };
}
...
export function evaluateBenchmarkRun(
  ...
  // SHORTCUT: 'stopped' runs are not evaluable, and a run with any cancelled
  // session can only reach 'complete' via resume mode:'retry' — so one user
  // stop permanently gates evaluation behind a retry. Paying this back =
  // decide whether a stopped run's produced sessions may be judged (the judge
  // already skips absent sessions) and either accept 'stopped' here or
  // document the retry requirement in BENCHMARK.md.
  ...
  if (rubricJudgeableSessions(run).judgeable.length === 0) {
    throw new OperationError(
      `No case in run "${input.runId}"'s snapshot has a rubric — nothing to judge. ` +
        `Add rubrics to the cases you want scored (benchmark_update_case with a rubric) ` +
        `and launch a new run; runs snapshot rubrics at launch.`,
      "benchmark_no_rubric",
    );
  }
  if (!ctx.scheduler) {
...
export function resumeBenchmarkEvaluation(
  ...
  // Mirror resumeBenchmarkRun's guards: a pass that never started or already
  // completed has no remaining work to resume.
  if (evaluation.status === "pending") {
    throw new OperationError(
      `Benchmark evaluation "${evaluationId}" is still starting.`,
      "benchmark_evaluation_already_running",
    );
  }
  if (evaluation.status === "complete") {
    throw new OperationError(
      `Benchmark evaluation "${evaluationId}" is already complete.`,
      "benchmark_evaluation_complete",
    );
  }
...
export interface BenchmarkEvaluationReport extends BenchmarkEvaluationRecord {
  ...
  /** Run sessions the pass should judge (completed sessions of rubric'd cases). */
...
export function getBenchmarkRunEvaluationReports(
  ...
  // A pass judges every completed run-session whose case has a rubric; it is
  // incomplete (and retryable) until each of those has a verdict. Rubric-less
  // sessions are skipped by design and reported as a count, never as errors.
  const { judgeable, skippedNoRubric } = rubricJudgeableSessions(run);
  const expectedSessions = judgeable.length;
...
async function runEvaluationCoordinator(
  ...
  try {
    ...
    // Only rubric'd cases are judged; the rest are skipped, not errors.
    const { judgeable } = rubricJudgeableSessions(run);
    ...
    if (done) {
      const judged = judgeable.filter((rs) => {
      ...
      // judgeable run-session was judged (rubric-less ones are skipped, not
      // errors), else 'error' (retry/resume can fill the rest).
      const hasCancelled = done.sessions.some((s) => s.status === "cancelled");
      const incomplete = judged < judgeable.length;
      updateBenchmarkEvaluation(db.connection, {
        ...done,
        ...
        error:
          !hasCancelled && incomplete
            ? (lastError ??
              `${judgeable.length - judged} of ${judgeable.length} judge sessions incomplete.`)
            : null,
        updatedAt: now(),
      });
    }
