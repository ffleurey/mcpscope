...
// benchmark_* operations keep using.
...
// The agent-facing rubric schema must be exactly the domain contract
// (positive id, non-empty description, non-negative points) — a looser copy
// here once let CLI/MCP persist rubrics the frontend surface rejects.
const rubricCriterionShape = rubricCriterionSchema;
...
export const benchmarkDeleteInputSchema = z.object({
  benchmark_id: z.string().describe("Benchmark to delete."),
});
export type BenchmarkDeleteInput = z.infer<typeof benchmarkDeleteInputSchema>;

export const benchmarkDeleteOutputSchema = {
  benchmark_id: z.string(),
  deleted: z.boolean(),
};

export const benchmarkDeleteOperation = {
  id: "benchmark_delete" as const,
  description:
    "Delete a benchmark blueprint and its cases. Past runs are independent snapshots and are kept.",
  schema: benchmarkDeleteInputSchema,
  outputSchema: benchmarkDeleteOutputSchema,
  async execute(ctx: OperationContext, input: BenchmarkDeleteInput) {
    deleteBenchmarkEntry(ctx.db, input.benchmark_id);
    return { benchmark_id: input.benchmark_id, deleted: true };
  },
};

// ── benchmark_delete_run ─────────────────────────────────────────────────────

export const benchmarkDeleteRunInputSchema = z.object({
  run_id: z.string().describe("Run to delete."),
});
export type BenchmarkDeleteRunInput = z.infer<
  typeof benchmarkDeleteRunInputSchema
>;

export const benchmarkDeleteRunOutputSchema = {
  run_id: z.string(),
  deleted: z.boolean(),
};

export const benchmarkDeleteRunOperation = {
  id: "benchmark_delete_run" as const,
  description:
    "Delete a benchmark run snapshot: its report, its produced sessions, and its evaluation "
    + "passes (judge sessions included). The benchmark blueprint is untouched. An active "
    + "(pending/running/paused) run must be stopped first.",
  schema: benchmarkDeleteRunInputSchema,
  outputSchema: benchmarkDeleteRunOutputSchema,
  async execute(ctx: OperationContext, input: BenchmarkDeleteRunInput) {
    deleteBenchmarkRunEntry(ctx.db, input.run_id);
    return { run_id: input.run_id, deleted: true };
  },
};

// ── benchmark_delete_evaluation ──────────────────────────────────────────────

export const benchmarkDeleteEvaluationInputSchema = z.object({
  evaluation_id: z.string().describe("Evaluation pass to delete."),
});
export type BenchmarkDeleteEvaluationInput = z.infer<
  typeof benchmarkDeleteEvaluationInputSchema
>;

export const benchmarkDeleteEvaluationOutputSchema = {
  evaluation_id: z.string(),
  deleted: z.boolean(),
};

export const benchmarkDeleteEvaluationOperation = {
  id: "benchmark_delete_evaluation" as const,
  description:
    "Delete one evaluation pass and the judge sessions it spawned. The run and its "
    + "sessions are kept. An active (pending/running/paused) evaluation must be stopped first.",
  schema: benchmarkDeleteEvaluationInputSchema,
  outputSchema: benchmarkDeleteEvaluationOutputSchema,
  async execute(ctx: OperationContext, input: BenchmarkDeleteEvaluationInput) {
    deleteBenchmarkEvaluationEntry(ctx.db, input.evaluation_id);
    return { evaluation_id: input.evaluation_id, deleted: true };
  },
};
...
export const benchmarkEvaluateOperation = {
  ...
  description:
    ...
    + "scores each session against its case rubric. Only sessions whose case has "
    + "a rubric are judged; the rest are skipped (reported as skipped_no_rubric). "
    + "Repeatable (0..N per run; compare judge models). Returns immediately; poll "
    + "benchmark_run_evaluations for scores.",
