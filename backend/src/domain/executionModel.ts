...
export const STEP_TYPE = {
  ...
  ANALYSIS_BENCHMARK_EVALUATION: stepTypeKey('analysis_benchmark_evaluation'),
} as const
 ...
 *
 * SHORTCUT: this vocabulary diverges from the persisted `sessionStatusValues`
 * in model.ts — 'complete' here is rejected by the sessions.status CHECK
 * constraint, so a Session implementation returning it cannot be persisted.
 * Paying this back = unify on one session-status enum (likely the persisted
 * one) and delete this parallel type.
 */
