...
describe('MCP tool names derived from backend operation IDs', () => {
  ...
  it('produces exactly one tool name per expected operation', () => {
    const names = operationList.map((op) => `${TOOL_PREFIX}${op.id}`)
    expect(names).toEqual(EXPECTED_OPERATION_IDS.map((id) => `mcpscope_${id}`))
  })
})

describe('backend operation catalog — every expected operation with execute', () => {
  it('catalog matches EXPECTED_OPERATION_IDS exactly', () => {
...
describe('CLI/MCP parity — backend operation catalog is the source of truth', () => {
  it('backend catalog contains exactly the shipped operations', () => {
...
describe('backend result shape contracts — snake_case throughout', () => {
  it('create result shape has snake_case fields (not camelCase)', () => {
    type CreateResult = Awaited<ReturnType<typeof createOperation.execute>>
  ...
  it('status result shape has active_turn (snake_case)', () => {
    type StatusResult = Awaited<ReturnType<typeof statusOperation.execute>>
  ...
  it('list result has api_version 1, sessions with snake_case fields', () => {
    ...
    const sessionFields: Array<keyof Session> = ['id', 'title', 'status', 'model', 'updated_at']
...
describe('MCP structured output — outputSchema defined for all operations', () => {
  ...
  it('status outputSchema has active_turn (snake_case)', () => {
    ...
    expect(statusOperation.outputSchema).not.toHaveProperty('activeTurn')
  })
})
...
describe('MCP HTTP endpoint execution', () => {
  it('responds to JSON-RPC tools/list and returns every mcpscope tool in the catalog', async () => {
    ...
    try {
      ...
      expect(toolNames).toEqual(EXPECTED_OPERATION_IDS.map((id) => `mcpscope_${id}`))
    } finally {
      await app.close()
