...
function makeTurnRecord(
  overrides: Partial<TurnRecord> &
    Pick<TurnRecord, "id" | "sessionId" | "turnNumber">,
): TurnRecord {
  return {
    id: overrides.id,
    ...
    contextTokensAfterCompaction:
      overrides.contextTokensAfterCompaction ?? null,
...
function makeRoundRecord(
  overrides: Partial<RoundRecord> &
    Pick<RoundRecord, "id" | "turnId" | "roundIndex">,
): RoundRecord {
  return {
    id: overrides.id,
...
function makeAnalysisState(
  overrides: Partial<AnalysisSessionState>,
...
describe("analysis workflow helpers", () => {
  ...
  it("bootstrap indexes tool-call packets with reasoning before and cross-round reasoning after", async () => {
    ...
    createSessionRecord(
      db.connection,
      makeSessionRecord({ id: "TARG", sessionType: "primary" }),
    );
    createSessionRecord(
      db.connection,
      makeSessionRecord({
    ...
    insertTurnRecord(
      db.connection,
      makeTurnRecord({ id: "TARG.1", sessionId: "TARG", turnNumber: 1 }),
    );
    insertRoundRecord(
      db.connection,
      makeRoundRecord({
    ...
    insertRoundRecord(
      db.connection,
      makeRoundRecord({ id: "TARG.1.2", turnId: "TARG.1", roundIndex: 1 }),
    );
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        payload: {
          text: null,
          json: { id: "call-1", name: "weather_get", arguments: "{}" },
          mimeType: "application/json",
          summary: null,
        },
      }),
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        payload: {
          text: null,
          json: { tool_call_id: "call-1", temperature_c: 21 },
          mimeType: "application/json",
          summary: null,
        },
      }),
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        roleLabel: "assistant",
        payload: {
          text: "The result answers the question.",
          json: null,
          mimeType: "text/plain",
          summary: null,
        },
  ...
  it("bootstrap filters packets by selected tool names and failed-only mode", async () => {
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        payload: {
          text: null,
          json: { tool_call_id: "call-1", ok: true },
          mimeType: "application/json",
          summary: null,
        },
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        payload: {
          text: null,
          json: { id: "call-2", name: "calendar_lookup", arguments: "{}" },
          mimeType: "application/json",
          summary: null,
        },
      }),
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        payload: {
          text: null,
          json: { tool_call_id: "call-2", error: "boom" },
          mimeType: "application/json",
          summary: null,
        },
  ...
  it("coverage validation derives completion from tool call ids plus accepted assessments", async () => {
    ...
    insertJsonArtifact(db.connection, {
      ...
      content: { ok: true },
      metadata: {
        schema_key: FULL_KEY.TOOL_CALL_ASSESSMENT,
        tool_call_part_id: "TC-1",
      },
      createdAt: 2,
    });

    const step = new CoverageValidationStep(
      db as unknown as BackendDatabase,
      null as unknown as ChatCompletionGateway,
      null as unknown as McpGateway,
      {
  ...
  it("coverage validation returns error when packets lack assessments", async () => {
    ...
    const diags = listArtifactsBySessionAndSchemaKey(
      db.connection,
      "ANLY",
      SCHEMA_KEY.DIAGNOSTIC,
    );
    expect(diags.length).toBeGreaterThan(0);
    expect(diags.some((d) => JSON.stringify(d.content).includes("TC-2"))).toBe(
      true,
    );
  });
  ...
  it("final aggregation still uses the LLM when multiple turn summaries need consolidation", async () => {
    ...
    insertStepRecord(
      ...
      makeStepRecord({
        ...
        sessionId: "ANLY",
        stepTypeKey:
          "analysis_final_aggregation" as StepPersistenceRecord["stepTypeKey"],
        childIndex: 0,
        status: "running",
      }),
    );
    ...
    insertJsonArtifact(db.connection, {
      ...
      content: {
        ...
        tool_call_assessment:
          "The tool call failed because the payload shape had to be corrected before success.",
        most_direct_cause: "wrong_parameters",
        parameter_or_call_issues: [
          "aggregation changed from array to scalar before success",
        ],
    ...
    insertJsonArtifact(db.connection, {
      ...
      content: {
        ...
        tool_call_assessment:
          "The tool call correctly requested the needed values.",
    ...
    insertJsonArtifact(db.connection, {
      ...
      content: {
        ...
        turn_outcome_rationale:
          "The turn answered the user request after a corrected retry path.",
        per_tool_findings: [
          {
            tool_call_part_id: "TC-1",
            tool_name: "test_tool",
            brief_finding:
              "The payload shape had to be corrected before success.",
          },
        ],
        cross_attempt_reconciliation:
          "The first turn only succeeded after correcting the payload shape from an array to a scalar value.",
      },
        ...
        per_tool_findings: [
          {
            ...
            tool_name: "test_tool",
            brief_finding:
              "The follow-up turn correctly requested the needed values.",
          },
        ],
          ...
          choices: [
            {
              ...
              message: {
                ...
                content: JSON.stringify({
                  outcome: "answered",
                  outcome_rationale:
                    "The workflow answered the request after consolidating both analyzed turns.",
                  primary_issue: "wrong_parameters",
                  primary_issue_rationale:
                    "The first turn required correcting the payload shape before succeeding.",
                  path_efficiency: "mixed",
                  path_efficiency_rationale:
                    "The workflow succeeded, but one turn required a corrective retry path.",
                  findings: [
                    "The first turn required correcting the payload shape before success.",
                    "The second turn was a clean success.",
                  ],
    ...
    const finalArtifact = getLatestArtifactBySchemaKey(
      db.connection,
      "ANLY",
      FULL_KEY.FINAL_ANALYSIS_REPORT,
    );
        ...
        per_tool_findings: [
          {
            tool_call_part_id: "TC-1",
            tool_name: "test_tool",
            brief_finding: "The first turn was a clean success.",
          },
        ],
        cross_attempt_reconciliation: null,
      },
        ...
        per_tool_findings: [
          {
            tool_call_part_id: "TC-2",
            tool_name: "test_tool",
            brief_finding: "The second turn was a clean success.",
          },
        ],
        cross_attempt_reconciliation: null,
      },
          ...
          choices: [
            {
              ...
              message: {
                ...
                content: JSON.stringify({
                  ...
                  path_efficiency_rationale:
                    "The workflow succeeded without corrective retries.",
                  findings: [
                    "The first turn was a clean success.",
                    "The second turn was a clean success.",
                  ],
                  tool_description_findings: [],
                  improvement_suggestions: [],
  ...
  it("repeated-attempt guidance avoids raw quoted JSON snippets", () => {
    ...
    insertRoundRecord(
      db.connection,
      makeRoundRecord({ id: "TARG.1.1", turnId: "TARG.1", roundIndex: 0 }),
    );
    insertRoundRecord(
      db.connection,
      makeRoundRecord({ id: "TARG.1.2", turnId: "TARG.1", roundIndex: 1 }),
    ...
    insertPartRecord(
      ...
      makePartRecord({
        ...
        payload: {
          text: null,
          json: {
            arguments: JSON.stringify({ aggregation: "max", interval: "day" }),
          },
          mimeType: null,
          summary: null,
        },
    ...
    const guidance = buildRepeatedAttemptGuidance(db, [
      {
        ...
        tool_call_parameters:
          '{"entity_ids":["sensor.temperature"],"interval":"hour"}',
        reasoning_before_part_id: null,
    ...
    expect(guidance).toContain(
      "aggregation: array(string(max)) -> string(max)",
    );
...
describe("analysis workflow registry", () => {
  it("allows new analysis types without touching shared files", () => {
    const ctor = {
      ...
      rehydrate: () => null as unknown as AnalysisSessionBase,
    ...
    expect(getWorkflowLabel("test_analysis_registry")).toBe(
