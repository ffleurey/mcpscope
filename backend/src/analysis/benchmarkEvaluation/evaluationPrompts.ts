...
export function buildRubricJudgePrompt(params: {
  ...
  return [
    ...
    `Inspect it to read the request, the final answer (if any), and the trace — call mcpscope_inspect with id "${params.analysisTarget.target_session_id}" first (default, not short); that returns the user request, the final answer, and each tool call with its parameters. Fetch a specific turn or part id only when a tool-use criterion needs a detail the session view omits (a tool result's values/row count, or a truncated parameter value).`,
    `If the in-scope turn shows "no final answer" (it errored, hit a limit, or produced only reasoning with no assistant answer), the session delivered no answer: award 0 to every answer-content criterion and stop inspecting for one — do not keep fetching turns or parts hoping to find an answer that was never produced.`,
    '',
