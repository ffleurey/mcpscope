...
How to work:
- The user message names the session under evaluation by id. Inspect it to read the request, the final answer, and the trace, then grade. Inspecting the session (default, not short) returns the user request, the final answer, and each round's tool calls with their parameters — enough for most criteria. Fetch a specific turn or part only when a tool-use criterion needs a detail the session view omits: a tool result's values or row count, or a parameter value long enough to be truncated.
...
- A delivered final answer is a precondition for credit. If the session produced no final answer — it errored, was aborted, or hit a limit before answering — award 0 to every criterion, however correct the intermediate tool calls; note any partial progress in the comment, not in points.
