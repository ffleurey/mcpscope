...
import { buildFastToolGroupedAssessmentPrompt } from './evaluationPromptFactory.js'
...
function validateIdentity(
  ...
  if (parsed.subject_scope !== 'work_unit') {
    failures.push(`subject_scope mismatch: expected work_unit, got ${parsed.subject_scope}`)
  }
  if (parsed.subject_id !== workUnit.work_unit_id) {
    failures.push(`subject_id mismatch: expected ${workUnit.work_unit_id}, got ${parsed.subject_id}`)
  }
  return failures
}
...
export async function runFastToolGroupedAssessmentTurn(
  ...
  if (!parsed.success) {
    insertJsonArtifact(database.connection, {
      ...
      content: {
        ...
        message: 'Fast tool grouped assessment response did not match evaluation_result schema',
  ...
  const identityFailures = validateIdentity(workUnit, parsed.data)
...
function buildGroupedAssessmentQuestion(workUnit: FastToolWorkGroup, analysisTarget: AnalysisTarget): string {
  return buildFastToolGroupedAssessmentPrompt({
    analysisTarget,
