...
describe('analysis plan derivation from artifacts', () => {
  ...
  it('isComplete distinguishes completed vs pending commands via artifact existence', () => {
    ...
    createSessionRecord(db.connection, {
      id: 'ANLY', title: 'ANLY', status: 'ready', initStatus: 'ready',
      sessionType: 'session_analysis', parentKind: 'session', parentId: 'TARG',
    ...
    insertStepRecord(db.connection, {
      id: 'step-1', sessionId: 'ANLY', stepTypeKey: 'analysis_bootstrap' as any,
    ...
    insertJsonArtifact(db.connection, {
      id: 'idx-1', sessionId: 'ANLY', stepId: 'step-1',
      content: { packets: [{ turn_id: 'TARG.1T', round_id: 'TARG.1.1', tool_call_part_id: 'TC-1', tool_name: 'test', tool_call_parameters: '{}', reasoning_before_part_id: null, tool_result_part_id: null, reasoning_after_part_id: null }] },
    ...
    const plan = [
      {
        kind: 'bootstrap', semanticId: '', stepTypeKey: '' as any,
        isComplete: (_db: any, _sid: string) => {
        ...
        buildStep: null as any,
      },
      {
        kind: 'assess', semanticId: 'TC-1', stepTypeKey: '' as any,
        isComplete: (_db: any, _sid: string) => {
    ...
    const firstIncomplete = plan.find(cmd => !cmd.isComplete(db as any, 'ANLY'))
