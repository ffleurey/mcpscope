...
import { capturedMultiTurnToolTrace } from "./testing/fixtures/capturedMultiTurnToolTrace.js";

const require = createRequire(import.meta.url);
const LightMyRequest =
  require("light-my-request/lib/request") as new (options: {
    url: string;
    method: string;
  }) => { socket: Record<string, unknown> };
const injectedSocketPrototype = Object.getPrototypeOf(
  new LightMyRequest({ url: "/", method: "GET" }).socket,
) as {
...
if (typeof injectedSocketPrototype.destroy !== "function") {
  injectedSocketPrototype.destroy = function destroy(error?: Error) {
    this.destroyed = true;
    if (error) {
      (this as { emit?: (event: string, value?: Error) => void }).emit?.(
        "error",
        error,
      );
    }
  ...
  return body
    ...
    .map((block) => {
      ...
      return {
        ...
        data: JSON.parse(dataLine?.slice(5).trim() ?? "{}") as Record<
          string,
          unknown
        >,
      };
...
describe("backend foundation", () => {
  ...
  it("lists sessions in reverse updated order and deletes them through the backend API", async () => {
    ...
    const firstSession = getSessionRecord(
    ...
    expect(
      listResponse.json().sessions.map((session: { id: string }) => session.id),
  ...
  it("stores backend-owned LM connections, model configs, and MCP profiles", async () => {
    ...
    const returnedProfiles = mcpProfilesResponse.json().mcpProfiles as Array<{
      id: string;
      source: string;
    }>;
    // Built-in companion profiles are always present and tagged read-only.
    expect(
      returnedProfiles.some(
        (p) => p.id === "builtin-open-meteo" && p.source === "builtin",
      ),
    ).toBe(true);
    // The user profile round-trips, tagged as a user profile.
    expect(returnedProfiles.find((p) => p.id === "mcp-1")).toEqual({
      ...mcpProfile,
      source: "user",
      disabledReason: null,
    });
  ...
  it("imports a captured multi-round trace bundle and re-exposes it through the canonical trace API", async () => {
    ...
    expect(traceBody.parts).toHaveLength(
      capturedReasoningThreeBatchParts.length,
    );
    expect(
      traceBody.transcript.filter(
        (entry: { type: string }) => entry.type === "assistant-reasoning",
      ),
    ).toHaveLength(4);
    expect(
      traceBody.context.some(
        (entry: { type: string }) => entry.type === "assistant-reasoning",
      ),
    ).toBe(false);
    expect(
      traceBody.parts.filter(
        (part: { partType: string }) => part.partType === "tool-call",
      ),
    ).toHaveLength(6);
    expect(
      traceBody.parts.filter(
        (part: { partType: string }) => part.partType === "tool-result",
      ),
    ).toHaveLength(6);
  });
  ...
  it("normalizes imported active execution state so trace imports cannot recreate a lock", async () => {
    ...
    const capturedTrace: SessionTraceBundle = {
      ...
      rounds: capturedReasoningThreeBatchRounds.map((round) =>
        round.roundIndex === 3
          ? {
              ...round,
              status: "streaming",
    ...
    expect(
      traceBody.rounds.find(
        (round: { roundIndex: number }) => round.roundIndex === 3,
      )?.status,
    ).toBe("aborted");
  ...
  it("imports deterministic compaction steps so they remain visible in trace and lookup APIs", async () => {
      ...
      parts: [
        ...
        },
        {
          ...
          context: {
            state: "stripped",
            note: null,
            strippedByCompactionAtTurnId: sourceTurnId,
          },
          tokens: {
            count: 48,
            source: "manual",
    ...
    expect(traceResponse.json()).toMatchObject({
      steps: expect.arrayContaining([
        expect.objectContaining({
          id: `${importedSessionId}.1T`,
          stepTypeKey: "turn",
          childIndex: expect.any(Number),
        }),
        expect.objectContaining({
    ...
    expect(
      traceResponse
        .json()
        .parts.some(
          (part: { id: string }) => part.id === `${importedSessionId}.1C.1-DN`,
        ),
    ).toBe(false);

    const sessionLookup = await app.inject({
      method: "GET",
    ...
    const stepSummary = await app.inject({
      method: "GET",
      url: `/api/lookup/${importedSessionId}.1C?mode=summary`,
    });
    ...
    const stepLookup = await app.inject({
      method: "GET",
      url: `/api/lookup/${importedSessionId}.1C?mode=full`,
    });
    ...
    const stepPartLookup = await app.inject({
      method: "GET",
      url: `/api/lookup/${importedSessionId}.1C.1-DN?mode=full`,
    });
  ...
  it("returns expected lookup payloads for session/turn/round/part on a multi-turn tool baseline", async () => {
    ...
    const baselineTrace: SessionTraceBundle = capturedMultiTurnToolTrace;
    const importResponse = await app.inject({
      method: "POST",
      url: "/api/traces/import",
    ...
    expect(traceResponse.statusCode).toBe(200);
    const traceBody = traceResponse.json();
    const turns = [...traceBody.turns].sort(
    ...
    const rounds = traceBody.rounds as Array<{
      id: string;
      turnId: string;
      roundIndex: number;
    }>;
    ...
    const firstRoundId = rounds.find(
      (round) => round.turnId === firstTurnId && round.roundIndex === 0,
    )?.id;
    const targetRound =
      rounds.find(
        (round) =>
          ...
          parts.filter(
            (part) =>
              part.roundId === round.id && part.partType === "tool-call",
          ).length > 0,
      ) ??
      rounds.find(
        (round) =>
    ...
    const toolCallPart = parts.find(
      (part) => part.roundId === targetRoundId && part.partType === "tool-call",
    );
    const toolCallPartId = toolCallPart?.id as string;
    const assistantContentPartId = parts.find(
      (part) =>
        part.roundId === targetRoundId && part.partType === "assistant-content",
    )?.id as string;
    const userPromptPartId = parts.find(
      (part) => part.partType === "user-message",
    )?.id as string;
    const setupPartId = parts.find(
      (part) => part.turnId === null && part.partType === "system-prompt",
    )?.id as string;
    const toolName = (toolCallPart?.payload.json?.toolName ??
      toolCallPart?.payload.json?.name) as string | undefined;
    expect(setupPartId).toBeDefined();
    expect(userPromptPartId).toBeDefined();

    const sessionSummary = await app.inject({
      method: "GET",
    ...
    expect(sessionSummary.json()).toMatchObject({
      ...
      data: {
        ...
        // available is null when the model's context window wasn't captured
        // (unloaded) — assert the field is present, not its value.
        context_window: expect.objectContaining({}),
    ...
    const sessionFull = await app.inject({
      method: "GET",
      url: `/api/lookup/${importedSessionId}?mode=full`,
    });
    ...
    const sessionFullSetupParts: { content?: unknown }[] =
      sessionFull.json().data.setup.parts;
    ...
    const turnFull = await app.inject({
      method: "GET",
    ...
    const turnFullParts: {
      ...
      .json()
      .data.rounds.flatMap((r: { parts: unknown[] }) => r.parts);
    expect(
      turnFullParts.some(
        (p) => p.type === "user_prompt" && p.content !== undefined,
      ),
    ).toBe(true);
    expect(turnFullParts.every((p) => p.tool_payload === undefined)).toBe(true);

    const firstRoundSummary = await app.inject({
      method: "GET",
    ...
    const roundSummary = await app.inject({
      method: "GET",
    ...
    const roundSummaryTool = roundSummary
      .json()
      .data.parts.find((p: { type: string }) => p.type === "tool_call");
    expect(roundSummaryTool.tool_payload).toBeUndefined();

    const roundFull = await app.inject({
      method: "GET",
    ...
    const roundFullTool = roundFull
      .json()
      .data.parts.find((p: { type: string }) => p.type === "tool_call");
    expect(roundFullTool.tool_payload).toEqual(
      expect.objectContaining({ call: expect.any(Object) }),
    );
    // no setup-type parts appear in round parts
    expect(
      roundFull
        .json()
        .data.parts.some((p: { type: string }) => p.type === "setup"),
    ).toBe(false);

    const partSummary = await app.inject({
      method: "GET",
    ...
    const partFull = await app.inject({
      method: "GET",
    ...
    if (assistantContentPartId) {
      const assistantPartSummary = await app.inject({
        method: "GET",
      ...
      const assistantPartFull = await app.inject({
        method: "GET",
    ...
    const userPromptPartSummary = await app.inject({
      method: "GET",
    ...
    const setupNodeFull = await app.inject({
      method: "GET",
    ...
    const setupNodeParts: {
      type: string;
      tools?: string[];
      content?: unknown;
    }[] = setupNodeFull.json().data.parts;
    const toolDefInSetup = setupNodeParts.find(
      (p) => p.type === "tool_definitions",
    );
    if (toolDefInSetup) {
    ...
    const toolDefPartId = parts.find(
      (part) => part.turnId === null && part.partType === "tool-definitions",
    )?.id;
    if (toolDefPartId) {
      const toolDefPartFull = await app.inject({
        method: "GET",
        url: `/api/lookup/${toolDefPartId}?mode=full`,
      });
    ...
    const setupPartSummary = await app.inject({
      method: "GET",
    ...
    const setupPartFull = await app.inject({
      method: "GET",
  ...
  it("streams model-only turn events as deltas followed by committed parts", async () => {
      ...
      chatCompletionGateway: {
        ...
        async streamChatCompletion(_baseUrl, _apiKey, _body, callbacks) {
          ...
          return {
            ...
            rawResponseBody:
              'data: {"choices":[{"delta":{"reasoning_content":"Because the answer is simple."}}]}\n\ndata: {"choices":[{"delta":{"content":"OK"}}]}\n\ndata: [DONE]\n',
            chunks: [],
          };
        },
      },
    ...
    expect(events[4]?.data.part).toEqual(
      expect.objectContaining({ partType: "assistant-reasoning" }),
    );
    expect(events[5]?.data.part).toEqual(
      expect.objectContaining({ partType: "assistant-content" }),
    );
    expect(
      (
        events.at(-1)?.data.trace as { context: Array<{ type: string }> }
      ).context.some((entry) => entry.type === "assistant-reasoning"),
    ).toBe(false);
  });
  ...
  it("streams tool-enabled turn events as deltas followed by committed parts", async () => {
      ...
      chatCompletionGateway: {
        ...
        async streamChatCompletion(_baseUrl, _apiKey, body, callbacks) {
                ...
                choices: [
                  {
                    ...
                    message: {
                      ...
                      content: null,
                      reasoning_content:
                        "I need the current time from the tool.",
                      tool_calls: [
                        {
                          id: "call-1",
                          type: "function",
          ...
          return {
            ...
            segments: [{ kind: "content", text: "The current time is 12:34." }],
            rawResponseBody:
              'data: {"choices":[{"delta":{"content":"The current time is 12:34."}}]}\n\ndata: [DONE]\n',
            chunks: [],
          };
        },
      },
        ...
        async listTools() {
          return {
            ...
            rawExchange: {
              ...
              responseStatus: 200,
              responseBody: {
                result: { tools: ["ha_history_get_current_time"] },
              },
            },
          };
        },
          ...
          return {
            ...
            rawExchange: {
              ...
              responseBody: {
                result: {
                  content: [
                    { type: "text", text: "2026-05-10T12:34:56+02:00" },
                  ],
                },
              },
            },
    ...
    expect(events[5]?.data.part).toEqual(
      expect.objectContaining({ partType: "tool-call" }),
    );
    expect(events[6]?.data.part).toEqual(
      expect.objectContaining({ partType: "tool-result" }),
    );
    expect(events[10]?.data.part).toEqual(
      expect.objectContaining({ partType: "assistant-content" }),
    );
    expect(
      (
  ...
  it("creates a session and completes a backend-owned model-only turn", async () => {
    ...
    expect(
      turnBody.parts.map((part: { partType: string }) => part.partType),
    ).toEqual(["user-message", "assistant-reasoning", "assistant-content"]);
    expect(
      getSessionRecord(app.backendDb.connection, createdSessionId)?.title,
    ).toBe("Integration Session");
    ...
    expect(
      traceBody.context.map((entry: { type: string }) => entry.type),
    ).toEqual(["system-prompt", "user-message", "assistant-content"]);
    expect(traceBody.turns).toHaveLength(1);
    expect(traceBody.rounds).toHaveLength(1);
    expect(
      traceBody.rawExchanges.map((exchange: { kind: string }) => exchange.kind),
    ).toEqual(
      expect.arrayContaining([
        "llm-probe-request",
        "llm-probe-response",
  ...
  it("completes a tool-enabled turn through the backend route", async () => {
    ...
    expect(
      body.parts.map((part: { partType: string }) => part.partType),
    ).toEqual([
      "user-message",
      "assistant-reasoning",
      "tool-call",
    ...
    expect(getSessionRecord(app.backendDb.connection, sessionId)?.title).toBe(
      "Tool session",
    );
    ...
    try {
      ...
      const response = await app.inject({
        ...
        payload: {
          ...
          mcpProfileSnapshots: [],
          selectedModel: {
            modelKey: "unloaded-model",
            modelDisplayName: "Unloaded Model",
          },
        },
      });
...
describe("session-creation-defaults API", () => {
  ...
  it("rejects unknown model config ID", async () => {
    ...
    const response = await app.inject({
      ...
      url: "/api/session-creation-defaults",
      payload: {
        defaultModelConfigId: "nonexistent",
  ...
  it("allows deleting model config and MCP profile that are not defaults", async () => {
    ...
    expect(
      (await app.inject({ method: "DELETE", url: "/api/mcp-profiles/mcp-1" }))
        .statusCode,
    ).toBe(204);
  });
});
...
describe("CLI session lifecycle endpoints", () => {
  ...
  const baseGateway = {
    chatCompletionGateway: {
      ...
      async streamChatCompletion(
        ...
        callbacks?: { onDelta?: (d: unknown) => void },
      ) {
  ...
  it("POST /api/sessions/from-defaults creates session with model and optional MCP", async () => {
    ...
    await app.inject({
      method: "PUT",
      url: "/api/session-creation-defaults",
      payload: { defaultModelConfigId: "mc-1" },
    });
  ...
  it("auto-titles an unnamed session from the first prompt only", async () => {
    ...
    const secondTurnRes = await app.inject({
      ...
      url: `/api/sessions/${sessionId}/turns`,
      payload: {
        userContent: "Second prompt must not replace the first auto title.",
      },
    });
  ...
  it("POST /api/sessions/from-defaults rejects duplicate session ID", async () => {
    ...
    const first = await app.inject({
      method: "POST",
      url: "/api/sessions/from-defaults",
      payload: { title: "A", sessionId: "AB23" },
    });
    expect(first.statusCode).toBe(201);
    ...
    const duplicate = await app.inject({
      method: "POST",
      url: "/api/sessions/from-defaults",
      payload: { title: "B", sessionId: "AB23" },
    });
    expect(duplicate.statusCode).toBe(409);
    expect(duplicate.json().error.code).toBe("duplicate_session_id");
  });
    ...
    app = (await buildBackendApp(config, {
      ...
      chatCompletionGateway: {
        ...
        async probePromptTokensDetailed(baseUrl, apiKey, body) {
          ...
          return baseGateway.chatCompletionGateway.probePromptTokensDetailed(
            baseUrl,
            apiKey,
            body,
          );
  ...
  describe("cross-session admission with queued execution", () => {
    ...
    function makeSessionInitializing(
      a: FastifyInstance,
      sessionId: string,
    ): void {
    ...
    it("POST /api/sessions is allowed when another session is initializing", async () => {
      ...
      const first = await app.inject({
        ...
        payload: {
          title: "Blocker",
          modelProfileSnapshot: minimalModelProfile,
        },
      });
      expect(first.statusCode).toBe(201);
    ...
    it("POST /api/sessions/:sessionId/turns/start is allowed when another session is initializing", async () => {
      ...
      const blockerSess = getSessionRecord(
        app.backendDb.connection,
        blockerId,
      )!;
...
describe("analysis launch", () => {
  ...
  async function createAnalysisModelConfig(
  ...
  function makeAnalysisMcpGateway(inspectIds: string[] = []) {
    ...
    return {
      async initializeSession(_url: string) {
        return {
          sessionId: "mcp-session-analysis",
          ...
          tools: [
            {
              ...
              description: "Inspect session objects",
              inputSchema: {
                type: "object",
   ...
   */
  function createCompleteTurnWithToolCall(
    appInst: FastifyInstance,
    sessionId: string,
  ): string {
    const ts = Date.now();
    ...
    insertPartRecord(appInst.backendDb.connection, {
      ...
      roleLabel: "assistant",
      payload: {
        text: "I should check the weather tool for Paris.",
        json: null,
        mimeType: "text/plain",
        summary: null,
      },
    ...
    insertPartRecord(appInst.backendDb.connection, {
      ...
      payload: {
        text: null,
        json: { tool_call_id: toolCallId, content: "Sunny, 22°C" },
        mimeType: "application/json",
        summary: "tool result",
      },
      display: { state: "transcript", collapsedByDefault: false },
    ...
    insertPartRecord(appInst.backendDb.connection, {
      ...
      roleLabel: "assistant",
      payload: {
        text: "The tool result is clear enough to answer directly.",
        json: null,
        mimeType: "text/plain",
        summary: null,
      },
  ...
  it("returns 404 when target session does not exist", async () => {
    ...
    const res = await appInst.inject({
      ...
      payload: {
        target_turn_id: "NONE-T1",
        analysis_goal: "Evaluate this session.",
      },
    });
  ...
  it("returns 422 when target session is not yet initialized", async () => {
    ...
    const res = await appInst.inject({
      ...
      payload: {
        model_config_id: "mc-1",
        target_turn_id: `${notReadySessionId}-T1`,
        analysis_goal: "Evaluate.",
      },
    });
  ...
  it("creates a session_analysis child session with correct parent link (v2 backend-owned workflow)", async () => {
    ...
    // Regression: the analysis session must carry the model config's context
    // window (like regular sessions). Dropping it made judge/analysis sessions
    // load at LM Studio's small JIT default, starving them into tool-loops.
    expect(stored?.modelProfileSnapshot.contextSize).toBe(40000);
    expect(stored?.modelProfileSnapshot.systemPrompt).toContain(
      "You are mcpscope's session analysis agent.",
    );
    expect(stored?.modelProfileSnapshot.systemPrompt).toContain(
      "Evaluate this session carefully.",
    );
    expect(stored?.modelProfileSnapshot.systemPrompt).toContain(
    ...
    const analysisState = storedSession?.analysisState as
      | { analysisGoal?: string }
      | undefined;
  ...
  it("GET /api/analysis/system-prompt-default returns the backend-owned default prompt", async () => {
    ...
    expect(res.json().systemPrompt).toContain(
      "You are mcpscope's session analysis agent.",
    );
    expect(res.json().systemPrompt).toContain(
    ...
    expect(res.json().systemPrompt).toContain(
      "When commenting on tool descriptions, quote or point to the specific wording",
    );
  });
  ...
  it("uses a launch-time system prompt override verbatim when supplied", async () => {
    ...
    const overridePrompt =
      "Custom analysis prompt override for this launch only.";
    const res = await app.inject({
      method: "POST",
      url: `/api/sessions/${targetId}/analyze`,
      payload: {
    ...
    expect(res.statusCode).toBe(201);
    const stored = getSessionRecord(
      app.backendDb.connection,
      res.json().session.id as string,
    );
  ...
  it("analysis child session is excluded from the default list but present with include_children", async () => {
    ...
    const childEntry = fullList
      .json()
      .sessions.find((s: { id: string }) => s.id === childId);
  ...
  it("analysis launch and queued execution are allowed while another session is running", async () => {
    ...
    const launchRes = await app.inject({
      ...
      payload: {
        model_config_id: "mc-1",
        target_turn_id: turnId,
        analysis_goal: "Queue this analysis.",
      },
    });
    expect(launchRes.statusCode).toBe(201);
    ...
    expect(enqueueRes.json().job.target).toMatchObject({
      kind: "session",
      sessionId: childId,
    });
  });
  ...
  it("analysis MCP endpoint is restricted to inspect and status tools only", async () => {
    ...
    // The restriction assertions must never pass vacuously: if the SSE parse
    // yields no tools, the response shape changed and this test must fail.
  ...
  it("v2 bootstrap creates analysis artifacts for a session with no tool calls", async () => {
    ...
    const res = await app.inject({
      ...
      payload: {
        model_config_id: "mc-1",
        target_turn_id: turnId,
        analysis_goal: "Did the session do the right thing?",
      },
    });
    ...
    expect(
      schemaKeys.filter((k) => k === "analysis.tool_call_assessment.v1"),
    ).toHaveLength(0);
  ...
  it("v2 full flow with tool calls: produces assessment, turn_summary, and final_report artifacts via deterministic inspect turns", async () => {
        ...
        async createChatCompletion() {
          ...
          if (idx === 0) {
            content = JSON.stringify({
              ...
              reasoning:
                "The selected tool matched the stated intent, and the city argument was set to Paris as expected.",
              verdict: "pass",
              score: 5,
    ...
    const launchRes = await app.inject({
      ...
      payload: {
        model_config_id: "mc-1",
        target_turn_id: turnId,
        analysis_goal: "Evaluate this tool call.",
      },
    });
    expect(launchRes.statusCode).toBe(201);
    const childId = launchRes.json().session.id as string;
    ...
    expect(deterministicParts.every((part) => part.token_count !== null)).toBe(
      true,
    );
  ...
  it("v2 fast session flow with tool calls: produces fast assessment, fast turn summary, and fast final report artifacts", async () => {
          ...
          const content =
            idx === 0
              ? JSON.stringify({
                  ...
                  evaluation_focus: "tool-call correctness",
                  reasoning:
                    "Tool was selected to retrieve the requested information.",
                  verdict: "pass",
                  score: 5,
                  evidence_part_id: `${turnRef.id}-P4`,
                })
              : idx === 1
                ? JSON.stringify({
                    ...
                    overall_rationale:
                      "The session answered the request directly.",
                    path_efficiency: "efficient",
                    tool_summaries: [
                      {
                        tool_name: "test_tool",
                ...
                : JSON.stringify({
                    overall_outcome: "answered",
                    overall_rationale:
                      "The session answered the request with one successful tool call.",
                    path_efficiency: "efficient",
                    tool_summaries: [
                      {
                        tool_name: "test_tool",
                        ...
                        inefficient_tool_calls: 0,
                        summary:
                          "The tool performed well in the assessed scope.",
                      },
                    ],
                    notable_failures: [],
                    follow_up_candidates: [],
    ...
    const schemaKeys = artifacts.map(
      (a) => (JSON.parse(a.metadata_json) as { schema_key: string }).schema_key,
    );

    expect(schemaKeys).toContain("analysis.analysis_target.v1");
    expect(schemaKeys).toContain("analysis.evidence_packet_index.v1");
    expect(schemaKeys).toContain(
      "analysis.fast_session_tool_call_assessment.v1",
    );
    expect(schemaKeys).toContain("analysis.fast_session_turn_summary.v1");
    expect(schemaKeys).toContain(
      "analysis.fast_session_final_analysis_report.v1",
    );
    expect(inspectIds).toEqual([
      targetId,
      `${turnId}-P2`,
  ...
  it("v2 fast session single-step execution does not emit analysis-complete before the workflow finishes", async () => {
    ...
    expect(schemaKeys).toContain("analysis.analysis_target.v1");
    expect(schemaKeys).toContain("analysis.evidence_packet_index.v1");
    expect(schemaKeys).not.toContain(
      "analysis.fast_session_final_analysis_report.v1",
    );

    const sessionRec = getSessionRecord(app.backendDb.connection, childId)!;
    const phase = (sessionRec.analysisState as { phase?: string } | null)
      ?.phase;
          ...
          const content =
            idx === 0
              ? JSON.stringify({
                  ...
                  reasoning:
                    "The grouped tool usage was effective for the target task.",
                  verdict: "pass",
                  score: 5,
              ...
              : JSON.stringify({
                  ...
                  overall_rationale:
                    "Tool use was strong and directly supported the answer.",
                  tool_summaries: [
                    {
      ...
      chatCompletionGateway: {
          ...
          const content =
            idx === 0
              ? JSON.stringify({
                  ...
                  evaluation_focus: "grouped tool usage",
                  reasoning:
                    "The tool was used effectively for the target task.",
                  verdict: "pass",
                  score: 5,
                  evidence_part_id: `${turnRef.id}-P3`,
                })
        ...
        async probePromptTokensDetailed(
          ...
          return {
            promptTokens,
            completion: {
              id: "probe-fast-tool-single-test",
              object: "chat.completion",
              created: Date.now(),
              model: "test-model",
              choices: [
  ...
  it("analysis execute rejects an assessment response whose identity does not match the expected packet", async () => {
            ...
            choices: [
              {
                ...
                message: {
                  ...
                  content: JSON.stringify({
                    ...
                    evaluation_focus: "tool-call correctness",
                    reasoning:
                      "The selected tool was plausible, but the invoked call did not match the expected packet identity.",
                    verdict: "partial",
                    score: 2,
    ...
    const artifacts = app.backendDb.connection
      .prepare(
        `SELECT metadata_json, content_json FROM artifacts WHERE session_id = ? ORDER BY created_at ASC`,
      )
      .all(childId) as Array<{ metadata_json: string; content_json: string }>;

    expect(
      artifacts.map((a) => JSON.parse(a.metadata_json).schema_key),
    ).not.toContain("analysis.tool_call_assessment.v1");
    expect(
      artifacts.map((a) => JSON.parse(a.metadata_json).schema_key),
    ).not.toContain("analysis.final_analysis_report.v1");
  ...
  it("analysis execute rejects a turn summary whose tool identities do not match the assessed turn", async () => {
    ...
    expect(
      artifacts.map((a) => JSON.parse(a.metadata_json).schema_key),
    ).toContain("analysis.tool_call_assessment.v1");
    expect(
      artifacts.map((a) => JSON.parse(a.metadata_json).schema_key),
    ).toContain("analysis.turn_summary.v1");
    expect(
      artifacts.map((a) => JSON.parse(a.metadata_json).schema_key),
    ).toContain("analysis.final_analysis_report.v1");
  ...
  it("pause stops analysis session execution after the current step and allows restart from the next step", async () => {
        ...
        async createChatCompletion() {
          ...
          if (idx === 0) {
            ...
            content = JSON.stringify({
              ...
              evaluation_focus: "tool-call correctness",
              reasoning:
                "The selected tool matched the stated intent and arguments.",
              verdict: "pass",
              score: 5,
              evidence_part_id: `${turnRef.id}-P4`,
            });
            ...
            content = JSON.stringify({
              ...
              reasoning:
                "The assessed tool call was appropriate and successful.",
              verdict: "pass",
              score: 5,
              evidence_part_id: `${turnRef.id}-P4`,
            });
    ...
    const launchRes = await app.inject({
      ...
      payload: {
        model_config_id: "mc-1",
        target_turn_id: turnId,
        analysis_goal: "Pause after the first assessed step.",
      },
    });
    expect(launchRes.statusCode).toBe(201);
    const childId = launchRes.json().session.id as string;
    ...
    const artifactsAfterPause = app.backendDb.connection
      .prepare(
        `SELECT metadata_json FROM artifacts WHERE session_id = ? ORDER BY created_at ASC`,
      )
      .all(childId) as Array<{ metadata_json: string }>;
    const schemaKeysAfterPause = artifactsAfterPause.map(
      (a) => (JSON.parse(a.metadata_json) as { schema_key: string }).schema_key,
    );
    ...
    expect(schemaKeysAfterPause).not.toContain(
    ...
    const artifactsAfterResume = app.backendDb.connection
      ...
      .all(childId) as Array<{ metadata_json: string }>;
    const schemaKeysAfterResume = artifactsAfterResume.map(
      (a) => (JSON.parse(a.metadata_json) as { schema_key: string }).schema_key,
    );
    expect(schemaKeysAfterResume).toContain("analysis.turn_summary.v1");
    expect(schemaKeysAfterResume).toContain(
      "analysis.final_analysis_report.v1",
    );
  });
