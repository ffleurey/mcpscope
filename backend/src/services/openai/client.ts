...
function appendToolCallSegment(
  ...
  const lastSegment = segments.at(-1);
  if (
    lastSegment?.kind === "tool-call" &&
    lastSegment.toolCallIndex === toolCallIndex
  ) {
    return;
  }
...
export function parseChatCompletionStream(
  ...
  for (const payload of payloads) {
    ...
    const rc = extractReasoningContent(
...
export async function streamChatCompletion(
  ...
  if (!response.body) {
    ...
    parsed.chunks.forEach((chunk) =>
      emitChunkDeltas(chunk, callbacks?.onDelta),
    );
    return parsed;
  }
  ...
  const processBufferedLines = () => {
    while (true) {
      ...
      if (line.length === 0) {
        ...
        if (payload !== "[DONE]") {
          emitChunkDeltas(
            JSON.parse(payload) as OaiChatCompletionChunk,
            callbacks?.onDelta,
          );
        }
        continue;
