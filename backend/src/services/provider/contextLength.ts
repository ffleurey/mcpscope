...
function buildOllamaModelKeyVariants(modelKey: string): string[] {
  ...
  // 3. If the key has a version-like suffix after ":"
  //    e.g. "qwen3.5:9b" → try "qwen3.5:latest"
  //    (already covered above via name.split)

  // 4. Try "latest" as a catch-all
  if (!modelKey.endsWith(":latest") && !variants.includes("latest")) {
    variants.push("latest");
  }
