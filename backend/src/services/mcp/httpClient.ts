...
/**
 * MCP protocol revision this client implements: streamable HTTP with JSON or
 * single-event SSE responses. Sent in `initialize`; the server may negotiate
 * down/up and its answer is echoed back on every follow-up request via the
 * `MCP-Protocol-Version` header (required since the 2025-03-26 revision).
 */
const MCP_PROTOCOL_VERSION = '2025-03-26'

/** clientInfo.version — the running mcpscope version (same source as config). */
function clientVersion(): string {
  return process.env.APP_VERSION ?? 'dev'
}

/** Negotiated protocol version per server URL, recorded at initialize. */
const negotiatedProtocolVersions = new Map<string, string>()
...
async function mcpPost(
  ...
  // Every request after initialize must carry the negotiated protocol version.
  if ((body as { method?: string }).method !== 'initialize') {
    requestHeaders['mcp-protocol-version'] =
      negotiatedProtocolVersions.get(url) ?? MCP_PROTOCOL_VERSION
  }
