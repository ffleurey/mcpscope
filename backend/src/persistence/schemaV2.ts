 ...
 *   - `v2_steps`            — generic step table (type key + params/state)
 *   - `v2_turns`            — LLM-specific extension linked to steps (infrastructure subtype)
 *   - `v2_rounds`           — Turn-owned rounds (linked to step_id, not turn_id)
 *   - `v2_parts`            — Turn-owned parts (linked to step_id)
...
export function initializeNewSchema(connection: Database.Database): void {
  connection.exec(`
    -- ─────────────────────────────────────────────────────────────────────
    -- Container ownership (Benchmark and future container types)
    -- ─────────────────────────────────────────────────────────────────────
    CREATE TABLE IF NOT EXISTS session_containers (
      id TEXT PRIMARY KEY,
      container_type_key TEXT NOT NULL,
      title TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_session_containers_type ON session_containers(container_type_key);
    ...
    -- ─────────────────────────────────────────────────────────────────────
    -- Turns (LLM-specific step extension — infrastructure subtype)
    -- ─────────────────────────────────────────────────────────────────────
    ...
    -- ─────────────────────────────────────────────────────────────────────
    -- Rounds (Turn-owned, linked to step_id)
    -- ─────────────────────────────────────────────────────────────────────
    CREATE TABLE IF NOT EXISTS v2_rounds (
      id TEXT PRIMARY KEY,
      step_id TEXT NOT NULL REFERENCES v2_steps(id) ON DELETE CASCADE,
      session_id TEXT NOT NULL REFERENCES v2_sessions(id) ON DELETE CASCADE,
      ...
      completed_at INTEGER,
      UNIQUE (step_id, round_index)
    );

    CREATE INDEX IF NOT EXISTS idx_v2_rounds_step_id ON v2_rounds(step_id);
    CREATE INDEX IF NOT EXISTS idx_v2_rounds_session_id ON v2_rounds(session_id);

    -- ─────────────────────────────────────────────────────────────────────
    -- Parts (Turn-owned, linked to step_id instead of turn_id)
    -- ─────────────────────────────────────────────────────────────────────
    CREATE TABLE IF NOT EXISTS v2_parts (
      id TEXT PRIMARY KEY,
      session_id TEXT NOT NULL REFERENCES v2_sessions(id) ON DELETE CASCADE,
      -- NULL for setup parts (not linked to any step)
    ...
    CREATE INDEX IF NOT EXISTS idx_v2_parts_step_id ON v2_parts(step_id);
    CREATE INDEX IF NOT EXISTS idx_v2_parts_round_id ON v2_parts(round_id);

    -- ─────────────────────────────────────────────────────────────────────
    -- Raw exchanges (diagnostics/replay layer, linked to step_id)
    -- ─────────────────────────────────────────────────────────────────────
    ...
    CREATE INDEX IF NOT EXISTS idx_v2_raw_exchanges_step_id ON v2_raw_exchanges(step_id);
    ...
    CREATE TABLE IF NOT EXISTS artifacts (
      id TEXT PRIMARY KEY,
      -- Owning session (optional — artifacts may be session-scoped or step-scoped)
      session_id TEXT REFERENCES v2_sessions(id) ON DELETE CASCADE,
      -- Owning step (optional — null for session-level artifacts)
      step_id TEXT REFERENCES v2_steps(id) ON DELETE CASCADE,
      artifact_type_key TEXT NOT NULL CHECK (artifact_type_key IN (${sqlEnum(artifactTypeValues)})),
      -- Content storage: exactly one of content_text / content_json / content_data is used
      content_text TEXT,   -- for 'text' and 'markdown' artifacts
...
export function validateNewSchema(connection: Database.Database): void {
  ...
  const required: Record<string, string[]> = {
    session_containers: ['id', 'container_type_key', 'title', 'params_json', 'state_json', 'created_at', 'updated_at'],
    v2_sessions: [
    ...
    v2_parts: [
      'id', 'session_id', 'step_id', 'round_id', 'parent_part_id',
    ...
    v2_raw_exchanges: [
      'id', 'session_id', 'step_id', 'round_id', 'kind',
