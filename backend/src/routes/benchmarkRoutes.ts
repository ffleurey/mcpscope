 ...
 * /api/operations/*. See BENCHMARK.md.
 */
export function registerBenchmarkRoutes({
  ...
  // Parse the request body through the operation's own input schema before
  // executing — the MCP adapter gets this from the SDK, and the HTTP path must
  // reject the same inputs (adapter parity). Invalid input is a structured 400,
  // never a persisted bad record or a deep runtime failure.
  const executeParsed = async <S extends z.ZodTypeAny, R>(
    op: {
      id: string;
      schema: S;
      execute(ctx: OperationContext, input: z.infer<S>): Promise<R>;
    },
    body: unknown,
  ): Promise<R> => {
    ...
    if (!parsed.success) {
      ...
      throw new OperationError(
        `Invalid input for ${op.id}: ${issues}`,
        "benchmark_invalid_input",
      );
    }
    return op.execute(opCtx, parsed.data as z.infer<S>);
  };

  app.post("/api/operations/benchmark-create", async (request, reply) => {
    try {
      return await executeParsed(benchmarkCreateOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post("/api/operations/benchmark-add-case", async (request, reply) => {
    try {
      return await executeParsed(benchmarkAddCaseOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        return await executeParsed(benchmarkAddCaseFromSessionOperation, request.body);
      } catch (err) {
        return handleOperationError(err, reply);
      }
    },
  ...
  app.post("/api/operations/benchmark-update-case", async (request, reply) => {
    try {
      return await executeParsed(benchmarkUpdateCaseOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post("/api/operations/benchmark-delete-case", async (request, reply) => {
    try {
      return await executeParsed(benchmarkDeleteCaseOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });

  app.post("/api/operations/benchmark-delete", async (request, reply) => {
    try {
      return await executeParsed(benchmarkDeleteOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post("/api/operations/benchmark-run", async (request, reply) => {
    try {
      return await executeParsed(benchmarkRunOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post("/api/operations/benchmark-run-control", async (request, reply) => {
    try {
      return await executeParsed(benchmarkRunControlOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });

  app.post("/api/operations/benchmark-delete-run", async (request, reply) => {
    try {
      return await executeParsed(benchmarkDeleteRunOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post("/api/operations/benchmark-evaluate", async (request, reply) => {
    try {
      return await executeParsed(benchmarkEvaluateOperation, request.body);
    } catch (err) {
      return handleOperationError(err, reply);
    }
  });
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        return await executeParsed(benchmarkEvaluationControlOperation, request.body);
      } catch (err) {
        return handleOperationError(err, reply);
      }
    },
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        return await executeParsed(benchmarkDeleteEvaluationOperation, request.body);
      } catch (err) {
        return handleOperationError(err, reply);
      }
    },
