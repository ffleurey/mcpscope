...
export function registerTraceRoutes({ app, database, scheduler, opCtx, handleOperationError, relaySchedulerJobStream }: RouteDeps): void {
  ...
  app.post('/api/sessions/:sessionId/turns', async (request, reply) => {
    ...
    // Client-disconnect handling: the turn belongs to the scheduler and always
    // runs to completion (same semantics as the non-blocking /turns/start
    // path) — but if the caller hung up while waiting, skip loading and
    // serializing a full trace response nobody will read.
