...
function makeSqlitePath() {
  return path.join(
    ".tmp-test-data",
    `replay-trace-${crypto.randomUUID()}`,
    "test.db",
  );
}
...
function parseRecordedLmResponse(
...
function normalizeTraceBundle(trace: SessionTraceBundle) {
  const turnRefById = new Map(
    trace.turns.map((turn) => [turn.id, `turn-${turn.turnNumber}`]),
  );
  const stepRefById = new Map(
    trace.steps.map((step, i) => [step.id, `step-${i + 1}`]),
  );
  const roundRefById = new Map(
    trace.rounds.map((round) => [
      round.id,
      `${turnRefById.get(round.turnId) ?? "turn-unknown"}-round-${round.roundIndex}`,
    ]),
  );
  const partRefById = new Map(
    trace.parts.map((part) => [part.id, `part-${part.ordinal}`]),
  );
  ...
  return {
    ...
    parts: trace.parts.map((part) => ({
      ...
      parentPartRef: part.parentPartId
        ? (partRefById.get(part.parentPartId) ?? null)
        : null,
      ...
      context: {
        ...
        strippedByCompactionAtTurnRef: part.context.strippedByCompactionAtTurnId
          ? (stepRefById.get(part.context.strippedByCompactionAtTurnId) ??
            ((): string => {
              const parts =
                part.context.strippedByCompactionAtTurnId.split(".");
              return parts.length >= 2
                ? parts[parts.length - 1]!
                : part.context.strippedByCompactionAtTurnId;
            })())
          : null,
      },
      tokens: part.tokens,
    ...
    rawExchanges: trace.rawExchanges
      .map((exchange) => ({
        turnRef: exchange.turnId
          ? (turnRefById.get(exchange.turnId) ?? null)
          : null,
        roundRef: exchange.roundId
          ? (roundRefById.get(exchange.roundId) ?? null)
          : null,
        kind: exchange.kind,
    ...
    transcript: trace.transcript.map((entry) => {
      const record = entry as {
        ...
        context?: {
          state?: unknown;
          note?: unknown;
          strippedByCompactionAtTurnId?: string | null;
        } | null;
      };
      return {
        turnRef: record.turnId
          ? (turnRefById.get(record.turnId) ?? null)
          : null,
        roundRef: record.roundId
          ? (roundRefById.get(record.roundId) ?? null)
          : null,
        ...
        context: record.context
          ? {
              ...
              strippedByCompactionAtTurnRef: record.context
                .strippedByCompactionAtTurnId
                ? (stepRefById.get(
                    record.context.strippedByCompactionAtTurnId,
                  ) ??
                  record.context.strippedByCompactionAtTurnId
                    .split(".")
                    .slice(-1)[0])
...
function createReplayChatCompletionGateway(
  ...
  return {
    async createChatCompletion() {
      throw new Error(
        "Replay harness expects streamed completions or recorded prompt probes only",
      );
    },
    async probePromptTokensDetailed(
      ...
      return {
        ...
        rawExchange: {
          ...
          requestHeadersJson: pair.request.requestHeadersJson as Record<
            string,
            string
          > | null,
          requestBody: pair.request.requestBody,
          responseStatus: pair.response.responseStatus,
          responseHeadersJson: pair.response.responseHeadersJson as Record<
            string,
            string
          > | null,
  ...
  return {
    async initializeSession(serverUrl) {
      ...
      return {
        sessionId:
          (
            pair.response.responseHeadersJson as {
        ...
        rawExchange: {
          requestUrl: pair.request.requestUrl,
          requestMethod: pair.request.requestMethod,
          requestHeaders: pair.request.requestHeadersJson as Record<
            string,
            string
          > | null,
          requestBodyText: pair.request.requestBody ?? "",
          responseStatus: pair.response.responseStatus ?? 200,
          responseHeaders: pair.response.responseHeadersJson as Record<
            string,
            string
          > | null,
  ...
  try {
    const createSessionResponse = await app.inject({
      ...
      payload: {
        ...
        // Propagate the captured compaction strategy — replay must not depend
        // on capture and replay sharing the same default (a trace recorded
        // with 'none' would otherwise fail the normalized-session comparison).
        ...(trace.session.compactionStrategy
          ? { compactionStrategy: trace.session.compactionStrategy }
          : {}),
      },
    });
    ...
    const userTurns = trace.turns
      ...
      .map((turn) => {
        const userPart = trace.parts
          .filter(
            (part) =>
              part.turnId === turn.id && part.partType === "user-message",
          )
          .sort((a, b) => a.ordinal - b.ordinal)[0];
        if (!userPart?.payload.text) {
          throw new Error(
            `Trace turn ${turn.turnNumber} is missing its user-message payload`,
          );
        }
...
export async function expectTraceToReplay(
  trace: SessionTraceBundle,
): Promise<void> {
