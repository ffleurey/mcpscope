...
function estimateDeterministicToolCallTokens(
  ...
  return {
    count: estimateTokenCountFromText(
      JSON.stringify({
        id: toolCall.id,
        name: toolCall.name,
  ...
  for (let roundIndex = 0; roundIndex < input.maxToolRounds; roundIndex++) {
    ...
    if (finishReason === "tool_calls") {
      ...
      for (const toolCallPart of toolCallParts) {
        ...
        const parsedArgs = JSON.parse(toolCall.argumentsJson || "{}") as Record<
          string,
          unknown
        >;
        const serverUrl = mcpContext.toolServerMap.get(toolCall.name);
        if (!serverUrl)
          throw new Error(`No MCP server found for tool "${toolCall.name}"`);
