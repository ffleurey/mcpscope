...
There is also a **restricted endpoint** at `/mcp/analysis` exposing only `inspect` + `status` —
used to give judge/analysis models read-only access to traces (see below).
...
| `mcpscope_benchmark_delete`             | `mcpscope benchmark_delete`                | Delete a benchmark blueprint and its cases (past runs are kept) |
...
| `mcpscope_benchmark_delete_run`         | `mcpscope benchmark_delete_run`            | Delete a run snapshot, its sessions, and its evaluation passes (stop an active run first) |
...
| `mcpscope_benchmark_delete_evaluation`  | `mcpscope benchmark_delete_evaluation`     | Delete one evaluation pass and its judge sessions (the run is kept) |
...
| `wait`           | boolean                           |          | Block until initialization finishes; `init_status` is then terminal (`ready`/`error`) — no status polling needed |
...
| `wait`       | boolean |         | Block until the turn is terminal; `turn.status` is then `complete`/`error`/`aborted` — no status polling needed |
...
| Field  | Type    | Required | Description |
|--------|---------|----------|-------------|
| `id`   | string  | ✓        | Any object ID. Runtime: `ABCD`, `ABCD.S`, `ABCD.1T`, `ABCD.4W` / `ABCD.2C` (step), `ABCD.4W.1T.2`, `ABCD.4W.1T.2.3-R`. Benchmark family: `B-XXXX` (benchmark), `B-XXXX.N` (case), `R-XXXX` (run; full mode adds the metrics report), `E-XXXX` (evaluation with scores). Inspecting a session, setup, step, turn, or round is useful for finding child IDs; inspect the returned part IDs directly for full evidence such as tool payloads, tool results, and part content. |
| `short`| boolean |          | Token counts only, no part content |
| `format`| `"text"` \| `"json"` | | Output rendering; default `"text"` (compact, human- and LLM-readable). `"json"` returns the full structured payload as JSON text. |
...
No inputs. Returns a list of all MCP server profiles. Each entry carries `id`, `name`, `url`,
`default_enabled`, `source`, and `disabled_reason`:

- `source` — `"builtin"` for a bundled [companion server](COMPANIONS.md) (read-only) or `"user"`
  for one you created.
...
### `mcpscope_benchmark_delete`

| Field          | Type   | Required | Description |
|----------------|--------|----------|-------------|
| `benchmark_id` | string | ✓        | Benchmark blueprint to delete (cases go with it; past runs are independent snapshots and are kept) |
...
### `mcpscope_benchmark_delete_run`

| Field    | Type   | Required | Description |
|----------|--------|----------|-------------|
| `run_id` | string | ✓        | Run to delete — removes the snapshot, its produced sessions, and its evaluation passes (judge sessions included). An active (pending/running/paused) run is refused (`benchmark_run_active`); stop it first |

### `mcpscope_benchmark_delete_evaluation`

| Field           | Type   | Required | Description |
|-----------------|--------|----------|-------------|
| `evaluation_id` | string | ✓        | Evaluation pass to delete — removes its judge sessions; the run and its sessions are kept. An active pass is refused (`benchmark_evaluation_active`); stop it first |
...
**One exception:** `mcpscope_inspect` renders its result as a single text `content` channel and
registers no `outputSchema` / `structuredContent` — its default output is the compact text view,
and `format: "json"` returns the structured payload as JSON text (never double-encoded).
...
The backend operation catalog is the single source of truth for both CLI and MCP — see [ARCHITECTURE.md → Backend-owned operation catalog](docs/ARCHITECTURE.md#backend-owned-operation-catalog).

To verify parity: `npm test` — the parity tests (`backend/src/mcp/mcp.test.ts`,
`cli/src/commands/commandCatalog.test.ts`) pin the operation ID list on both surfaces: every
catalog operation must exist as an MCP tool and as a CLI command, and each operation must define
an input schema and an output schema. The MCP side is generated mechanically from the catalog, so
its schemas cannot drift; the CLI's hand-written flag parsers are held to the catalog by these
ID-level checks and by review, not by automated schema comparison. Known, documented per-surface
differences are listed in [CLI.md](CLI.md) (e.g. `benchmark_add_case` rubric authoring and the
